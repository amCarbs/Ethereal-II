...
public final class RegionBuilder {
	...
	public static final void destroyMap(int toRegionX, int toRegionY,
		...
		for (int xOffset = 0; xOffset < widthRegions; xOffset++) {
			for (int yOffset = 0; yOffset < heightRegions; yOffset++) {
				...
				destroyDynamicRegion(((toThisRegionX / 8) << 8)
						+ (toThisRegionY / 8));
			}
		}
	}

	public static final void repeatMap(int toRegionX, int toRegionY,
			int widthRegions, int heightRegions, int rx, int ry, int plane,
	...
	public static final void cutMap(int toRegionX, int toRegionY,
			int widthRegions, int heightRegions, int... toPlanes) {
		for (int xOffset = 0; xOffset < widthRegions; xOffset++) {
			for (int yOffset = 0; yOffset < heightRegions; yOffset++) {
				int toThisRegionX = toRegionX + xOffset;
				int toThisRegionY = toRegionY + yOffset;
	 ...
	 */
	public static void copyRegion(int fromRegionX, int fromRegionY,
			int fromPlane, int toRegionX, int toRegionY, int toPlane,
			int rotation) {
		...
		for (int xOffset = 0; xOffset < widthRegions; xOffset++) {
			for (int yOffset = 0; yOffset < heightRegions; yOffset++) {
				...
				for (int pIndex = 0; pIndex < fromPlanes.length; pIndex++) {
					...
					World.getRegion(
							(((fromThisRegionX / 8) << 8) + (fromThisRegionY / 8)),
							true);
				}
			}
		}
	...
	public static final void copy2RatioSquare(int fromRegionX, int fromRegionY,
		...
		if (rotation == 0) {
			copyRegion(fromRegionX, fromRegionY, 0, toRegionX, toRegionY, 0,
					rotation);
			copyRegion(fromRegionX + 1, fromRegionY, 0, toRegionX + 1,
					toRegionY, 0, rotation);
			copyRegion(fromRegionX, fromRegionY + 1, 0, toRegionX,
					toRegionY + 1, 0, rotation);
			copyRegion(fromRegionX + 1, fromRegionY + 1, 0, toRegionX + 1,
					toRegionY + 1, 0, rotation);
		} else if (rotation == 1) {
			copyRegion(fromRegionX, fromRegionY, 0, toRegionX, toRegionY + 1,
					0, rotation);
			copyRegion(fromRegionX + 1, fromRegionY, 0, toRegionX, toRegionY,
					0, rotation);
			copyRegion(fromRegionX, fromRegionY + 1, 0, toRegionX + 1,
					toRegionY + 1, 0, rotation);
			copyRegion(fromRegionX + 1, fromRegionY + 1, 0, toRegionX + 1,
					toRegionY, 0, rotation);
		} else if (rotation == 2) {
			copyRegion(fromRegionX, fromRegionY, 0, toRegionX + 1,
					toRegionY + 1, 0, rotation);
			copyRegion(fromRegionX + 1, fromRegionY, 0, toRegionX,
					toRegionY + 1, 0, rotation);
			copyRegion(fromRegionX, fromRegionY + 1, 0, toRegionX + 1,
					toRegionY, 0, rotation);
			copyRegion(fromRegionX + 1, fromRegionY + 1, 0, toRegionX,
					toRegionY, 0, rotation);
		} else if (rotation == 3) {
			copyRegion(fromRegionX, fromRegionY, 0, toRegionX + 1, toRegionY,
					0, rotation);
			copyRegion(fromRegionX + 1, fromRegionY, 0, toRegionX + 1,
					toRegionY + 1, 0, rotation);
			copyRegion(fromRegionX, fromRegionY + 1, 0, toRegionX, toRegionY,
					0, rotation);
			copyRegion(fromRegionX + 1, fromRegionY + 1, 0, toRegionX,
					toRegionY + 1, 0, rotation);
		}
