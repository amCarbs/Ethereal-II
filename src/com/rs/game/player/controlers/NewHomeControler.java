...
public class NewHomeControler extends Controler {
	...
	private static Item[] STARTER_ITEMS = { new Item(1, 1) };// TODO
	...
	private void getDroppedItems() {
		...
		for (Item item : STARTER_ITEMS)
			World.addGroundItem(item, NPC1.getLastWorldTile(), player, true,
					120 * 1000, true);
		player.setCoordsEvent(new CoordsEvent(NPC1.getLastWorldTile(),
				new Runnable() {
					@Override
					public void run() {
						...
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								...
								player.getDialogueManager()
										.startDialogue("SimpleNPCMessage", 1,
												"Please sir, help me capture the man who stole my belongings");
								player.getHintIconsManager().addHintIcon(NPC1,
										0, -1, false);
								player.resetStopDelay();
							}
						}, 2);
