...
public class ClanWarsControler extends Controler {
	...
	public void start() {
		...
		if (player.getCurrentFriendChat().isChallenger()) {
			player.setNextWorldTile(new WorldTile(war.getBaseX()
					+ war.getArena().getIncreaseInTeamAX(), war.getBaseY()
					+ war.getArena().getIncreaseInTeamAY(), 0));
			war.getChallengerTeam().add(player);
		} else {
			player.setNextWorldTile(new WorldTile(war.getBaseX()
					+ war.getArena().getIncreaseInTeamBX(), war.getBaseY()
					+ war.getArena().getIncreaseInTeamBY(), 0));
	...
	public boolean processButtonClick(int interfaceId, int componentId,
		...
		case 271:
			if (war.isSet(Rule.BAN_PRAY)) {
				player.getPackets().sendGameMessage(
						"You can't use prayers during this war.");
				return false;
			}
			return true;
		}
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		player.getDialogueManager()
				.startDialogue("SimpleDialouge",
						"You can't teleport from this battle. Use the portals you came from to leave.");
		return false;
	}
	...
	private void updatePlayers(Player player) {
		if (player.getCurrentFriendChat().isChallenger()) {
			for (Player players : war.getChallengerTeam()) {
				players.getPackets().sendIComponentText(265, 5,
						Integer.toString(war.getChallengerTeam().size()));
				players.getPackets().sendIComponentText(265, 11,
						Integer.toString(war.getOpponentTeam().size()));
				players.getPackets().sendIComponentText(265, 7,
						Integer.toString(war.getChallengerTeamKills().size()));
				players.getPackets().sendIComponentText(265, 13,
						Integer.toString(war.getOpponentTeamKills().size()));
			}
		} else {
	...
	private void teleportPlayer(Player player) {
		if (player.getCurrentFriendChat().isChallenger())
			player.setNextWorldTile(new WorldTile(war.getBaseX()
					+ war.getArena().getChallengerDeathLocX(), war.getBaseY()
					+ war.getArena().getChallengerDeathLocY(), 0));
		else
			player.setNextWorldTile(new WorldTile(war.getBaseX()
					+ war.getArena().getOpponentDeathLocX(), war.getBaseY()
					+ war.getArena().getOpponentDeathLocY(), 0));
	}
