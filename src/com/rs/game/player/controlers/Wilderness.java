...
return true;
player.getPackets().sendGameMessage(
"The combat difference between that player is to great." );
...
showSkull();
	...
	public boolean sendDeath() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					player.setNextAnimation(new Animation(15524));//836
				...
				//	if (killer != null) {
					...
					//	if (player.getSession().getIP().equals(killer.getSession().getIP())) {
						//		player.getPackets().sendGameMessage("You cannot Gain Killcount with someone on the same ip.");
						//		player.getPackets().sendGameMessage("You cannot Gain Killcount with someone on the same ip.");
					//	}
					//	else{
...
public static final boolean isAtWild(WorldTile tile) {
	return 
		...
		&& tile.getY() >= 10052 && tile.getY() <= 10175) || //real revs
		...
		//danger clan
		|| (tile.getX() >= 2948 && tile.getX() <= 3069
		&& tile.getY() >= 5512 && tile.getY() <= 5630)
		//dm arena
		|| (tile.getX() >= 2441 && tile.getX() <= 2482
		&& tile.getY() >= 4237 && tile.getY() <= 4283)
		;
		}
...
public boolean isAtWildSafe() {
	return (player.getX() >= 2940 && player.getX() <= 3395
			...
			|| (player.getX() >= 3084 && player.getX() <= 3087
			&& player.getY() <= 3936 && player.getY() >= 3931)
			//clan
			|| (player.getX() >= 2948 && player.getX() <= 3070
			&& player.getY() <= 5511 && player.getY() >= 5509);
}
