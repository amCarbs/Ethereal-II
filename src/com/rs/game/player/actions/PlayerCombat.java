...
public class PlayerCombat extends Action {
		...
		if (spellId > 0) {
			...
			if (isRanging == 0) {
				...
				player.getPackets().sendGameMessage(
						"This ammo is not very effective with this.");
				return -1;
			} else if (isRanging == 3) {
				player.getPackets().sendGameMessage(
				...
				if (weaponName.contains("throwing axe")
					...
					delayHit(
							...
							getRangeHit(
									...
									getRandomMaxHit(player, weaponId,
											attackStyle, true)));
				} else if (weaponName.contains("polypore staff")) {
					...
					delayMagicHit(
							2,
							getMagicHit(player,
									getRandomMagicMaxHit(player, 585)));
					World.sendProjectile(player, target, 2035, 18, 18, 50, 50,
							0, 0);
					return 5;
				} else { // bow/default
	 ...
	 * ED *= 1.15; if (player.getPolDelay() >= Utils.currentTimeMillis()) ED *=
	...
	private void delayHit(int delay, final int weaponId, final int attackStyle,
		...
		for (Hit hit : hits) {
			...
			if (hit.getLook() == HitLook.RANGE_DAMAGE
				...
				double combatXp = (damage / 10) * 4.2342;
				if (combatXp > 0) {
	...
	private boolean checkAll(Player player) {
		...
		if (target instanceof Player) {
			...
			if (p2.getTemporaryAttributtes().get("InstantSpecial") != null) {
				int specialId = (int) p2.getTemporaryAttributtes().get(
						"InstantSpecial");
				if (specialId != p2.getEquipment().getWeaponId())
					return true;
				if (p2.getCombatDefinitions().getAutoCastSpell() > 0)
					return true;
				p2.getActionManager().setActionDelay(0);
				p2.getTemporaryAttributtes().remove("InstantSpecial");
				return true;
			}
		} else {
			NPC n = (NPC) target;
			if (!Slayer.checkRequirement(player,
					SlayerMonsters.forId(n.getId()))) {
				return false;
			}
			if (n.isCantInteract())
				return false;
