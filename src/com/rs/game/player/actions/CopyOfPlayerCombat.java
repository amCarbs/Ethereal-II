...
public class CopyOfPlayerCombat extends Action {
	...
	public CopyOfPlayerCombat(Entity target) {
		this.target = target;
	}
		...
		if (spellId > 0) {
			...
			if (isRanging == 0) {
				...
				player.getPackets().sendGameMessage(
						"This ammo is not very effective with pon.");
				return -1;
			} else if (isRanging == 3) {
				player.getPackets().sendGameMessage(
	...
	public int mageAttack(Player player, int spellId, boolean autocast) {
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 34: // air bolt
				...
				base_mage_xp = 13.5;
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 90)));
				World.sendProjectile(player, target, 2699, 18, 18, 50, 50, 0, 0);
				return 5;
			case 39: // water bolt
				...
				base_mage_xp = 16.5;
				delayMagicHit(2,
						getMagicHit(player, getRandomMagicMaxHit(player, 100)));
	 ...
	 * getBonuses()[CombatDefinitions.MAGIC_ATTACK]; att *=
	 * player.getPrayer().getMageMultiplier(); if (fullVoidEquipped(player,
	 * 11663, 11674)) att *= 1.3; double def; if (target instanceof Player) {
	 * Player p2 = (Player) target; def =
	 * p2.getSkills().getLevel(Skills.DEFENCE) +
	 * (p2.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_DEF] *
	 * 2); def *= p2.getPrayer().getDefenceMultiplier(); } else { NPC n = (NPC)
	 * target; def = n.getBonuses() == null ? 0 :
	 * n.getBonuses()[CombatDefinitions.MAGIC_DEF]; } double prob = att / def;
	 * if (prob > 0.90) // max, 90% prob hit so even lvl 138 can miss at lvl 3
	 * prob = 0.90; else if (prob < 0.05) // minimun 5% so even lvl 3 can hit
	 * lvl 138 prob = 0.05; if (prob < Math.random()) return 0; double
	 * extraDamage =
	 * player.getCombatDefinitions().getBonuses()[CombatDefinitions
	 * .MAGIC_DAMAGE]; max_hit = (int) (baseDamage * ((double) (1.0 +
	 * extraDamage / 100))); // max_hit *=
	 * player.getPrayer().getMageMultiplier(); double boost = 1 +
	 * ((player.getSkills().getLevel(Skills.MAGIC) - player
	 * .getSkills().getLevelForXp(Skills.MAGIC)) * 0.03); if (boost > 1) max_hit
	 * *= boost; return max_hit; }
	 */

	public int getMagicMaxHit(Player player, int baseDamage) {
		double defBonus = 0;
		double effectiveMagicDefence = 0;
		double att = ((player.getSkills().getLevel(Skills.MAGIC) + 8) * player
				.getPrayer().getMageMultiplier());
		...
		if (Utils.getRandomGenerator().nextDouble() > accuracy)
			return 0;
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe")
					...
					World.sendProjectile(player, target, 249, 41, 41, 41, 35,
							0, 0);
					delayHit(
							2,
							weaponId,
	 ...
	 * usingSpec, specMultiplier); if (defenceAffects) { double att =
	 * player.getSkills().getLevel(ranging ? 4 : 0) +
	 * player.getCombatDefinitions().getBonuses()[ranging ? 4 :
	 * CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle)]; att *=
	 * ranging ? player.getPrayer().getRangeMultiplier() : player
	 * .getPrayer().getAttackMultiplier(); if (fullVoidEquipped(player, ranging
	 * ? (new int[] { 11664, 11675 }) : (new int[] { 11665, 11676 }))) att *=
	 * 1.1; if (ranging) att *=
	 ...
	 * p2.getSkills().getLevel(Skills.DEFENCE) + (ranging ? 1.25D : 1.55D) + (2
	 ...
	 * .getMeleeBonusStyle(weaponId, attackStyle))] : 0; } if (usingSpec) {
	 * double multiplier = 0.25 + specMultiplier; att *= multiplier; } double
	 * prob = att / def; if (prob > 0.90) // max, 90% prob hit so even lvl 138
	 * can miss at // lvl 3 prob = 0.90; else if (prob < 0.05) // minimun 5% so
	 * even lvl 3 can hit lvl 138 prob = 0.05; if (prob < Math.random()) return
	 * 0; } int hit = Utils.getRandom(max_hit); if (target instanceof NPC) { NPC
	 * n = (NPC) target; if (n.getId() == 9463 && hasFireCape(player)) hit +=
	 * 40; } if (player.getAuraManager().usingEquilibrium()) { int perc25MaxHit
	 * = (int) (max_hit * 0.25); hit -= perc25MaxHit; max_hit -= perc25MaxHit;
	 * if (hit < 0) hit = 0; if (hit < perc25MaxHit) hit += perc25MaxHit;
	 * 
	 * } return hit; }
	 */
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle,
		...
		max_hit = getMaxHit(player, weaponId, attackStyle, ranging, usingSpec,
				specMultiplier);
		if (defenceAffects) {
			double EA = player.getSkills().getLevel(
					ranging ? Skills.RANGE : Skills.ATTACK);
			EA *= ranging ? player.getPrayer().getRangeMultiplier() : player
					.getPrayer().getAttackMultiplier();
			EA += 8;
			int style = CombatDefinitions.getXpStyle(weaponId, player
					.getCombatDefinitions().getAttackStyle());
			EA += style == Skills.ATTACK ? 3
					: style == CombatDefinitions.SHARED ? 1 : 0;
			if (fullVoidEquipped(player, ranging ? new int[] { 11664, 11675 }
					: new int[] { 11665, 11676 }))
				EA *= 1.1;
			if (ranging)
				EA *= player.getAuraManager().getRangeAccurayMultiplier();
			Math.floor(EA);
			double ED;
			double defBonus = 0;
			if (target instanceof Player) {
				Player p2 = (Player) target;
				ED = p2.getSkills().getLevel(Skills.DEFENCE);
				ED *= p2.getPrayer().getDefenceMultiplier();
				ED += 8;
				int p2style = CombatDefinitions.getXpStyle(p2.getEquipment()
						.getWeaponId(), p2.getCombatDefinitions()
						.getAttackStyle());
				ED += p2style == Skills.DEFENCE ? 3
						: p2style == CombatDefinitions.SHARED ? 1 : 0;
				defBonus = p2.getCombatDefinitions().getBonuses()[ranging ? CombatDefinitions.RANGE_DEF
						: CombatDefinitions
				...
				NPC n = (NPC) target;
				ED = n.getBonuses() == null ? 0
						: n.getBonuses()[ranging ? CombatDefinitions.RANGE_DEF
								: CombatDefinitions
										.getMeleeDefenceBonus(CombatDefinitions
												.getMeleeBonusStyle(weaponId,
														attackStyle))];
			...
			double A = EA
					* (1 + (player.getCombatDefinitions().getBonuses()[ranging ? CombatDefinitions.RANGE_ATTACK
							: CombatDefinitions.getMeleeBonusStyle(weaponId,
									attackStyle)] / 64.0D));
			A *= getSpecialAccuracyModifier(player);
			double D = ED * (1 + (defBonus / 64.0D));
			Math.floor(A);
			Math.floor(D);
			double accuracy = 0.05;
			if (A < D)
	...
	private double getSpecialAccuracyModifier(Player player) {
		...
		if (weapon == null)
			return 1;
		String name = weapon.getDefinitions().getName();
		if (name.contains("whip") || name.contains("dragon dagger")
				|| name.contains("dragon longsword")
				|| name.contains("dragon scimitar")
				|| name.contains("magic shortbow"))
			return 1.1;
	...
	private boolean checkAll(Player player) {
		...
		if (player.getTemporaryAttributtes().get("InstantSpecial") != null) {
			int specialId = (int) player.getTemporaryAttributtes().get(
					"InstantSpecial");
			if (specialId != player.getEquipment().getWeaponId())
				return true;
			if (player.getCombatDefinitions().getAutoCastSpell() > 0)
				return true;
			player.getActionManager().setActionDelay(0);
			player.getTemporaryAttributtes().remove("InstantSpecial");
			return true;
		}
		if (target instanceof Player) {
			Player p2 = (Player) target;
			...
			if (!player.addWalkSteps(target.getX() + 1, target.getY(), 1))
				if (!player.addWalkSteps(target.getX() - 1, target.getY(), 1))
					if (!player.addWalkSteps(target.getX(), target.getY() + 1,
							1))
						if (!player.addWalkSteps(target.getX(),
								target.getY() - 1, 1))
							return false;
		...
		maxDistance = isRanging != 0
				|| player.getCombatDefinitions().getSpellId() > 0 ? 7 : 0;
		if ((!player.clipedProjectile(target,
				target instanceof NexMinion ? false : maxDistance == 0))
				|| distanceX > size + maxDistance
				|| distanceX < -1 - maxDistance
