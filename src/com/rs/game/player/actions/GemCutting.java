...
public class GemCutting extends Action {
	 ...
	 */
	public enum Gem {
		OPAL(1625, 1609, 15.0, 1, 886), JADE(1627, 1611, 20, 13, 886), RED_TOPAZ(
				1629, 1613, 25, 16, 887), SAPPHIRE(1623, 1607, 25, 1, 888), EMERALD(
				1621, 1605, 50, 27, 889), RUBY(1619, 1603, 75, 34, 887), DIAMOND(
				1617, 1601, 107.5, 43, 890), DRAGONSTONE(1631, 1615, 137.5, 55,
				885), ONYX(6571, 6573, 168, 67, 2717);
	...
	public static void cut(Player player, Gem gem) {
		if (player.getInventory().getItems()
				.getNumberOf(new Item(gem.getUncut(), 1)) <= 1) // contains just
																// 1 lets start
			player.getActionManager().setSkill(new GemCutting(gem, 1));
		else
