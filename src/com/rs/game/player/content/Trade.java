...
public class Trade implements Serializable {
	...
	public static final int[] LENDABLE_ITEMS = { 4151 };
	 ...
	 * @param player
	 */
	public Trade(Player trader, Player partner, Player player) {
		this.trader = trader;
	...
	public void handleCollectButton(Player player) {
		Player target = World.getPlayerByIndex(player.getTrade().lentTo);
			...
			target.getPackets()
					.sendGameMessage(
							player.getDisplayName()
									+ "has claimed an item you borrowed and it has been returned.");
			// target.getTrade().removeAllBorrowedItems(target); //TODO
			player.getPackets().sendIComponentText(109, 59,
					"<col=00FF00>Claim!");
			player.getTrade().lentTo = -1;
			player.getTrade().lending = false;
			player.getInventory().addItem(player.getTrade().lentItem, 1);
			player.getTrade().lentItem = -1;
			player.getTrade().lending = false;
			Player oldTarget = target;
			oldTarget.getTrade().borrowing = false;
			player.closeInterfaces();
		} else if (player.getTrade().lentItem != -1) {
			for (int i = 540; i < 541; i++) {
				for (int j = player.getTrade().lentItem; j < player.getTrade().lentItem + 1; j++) {
	...
	public void addLendItem(int slotId) {
		...
		partner.getPackets().sendItems(541, true, player.getTrade().lendItems);
	...
	private int getTradersItemsValue() {
		...
		for (Item item : traderItemsOffered.toArray()) {
			if (item != null) {
				initialPrice += item.getDefinitions().getValue(item.getId())
						* item.getAmount();
			}
		}
		return initialPrice;
	...
	public void tradeFailed() {
		...
		partner.getPackets().sendGameMessage(
				"<col=FFF0000>Other player declined trade!</col>");
		traderItemsOffered = null;
		partnerItemsOffered = null;
		trader.setCloseInterfacesEvent(null);
		partner.setCloseInterfacesEvent(null);
