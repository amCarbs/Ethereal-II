...
public final class DungeonUtils {
	...
	public static int getSafeMusic(int type) {
		return DungeonConstants.SAFE_MUSICS[type][Utils
				.random(DungeonConstants.DANGEROUS_MUSICS[type].length)];
	}

	public static int getDangerousMusic(int type) {
		return DungeonConstants.DANGEROUS_MUSICS[type][Utils
				.random(DungeonConstants.DANGEROUS_MUSICS[type].length)];
	}

	public static RoomReference getRandomRoom(Room[][] map) {
		return new RoomReference(Utils.random(map.length),
				Utils.random(map[0].length));
	}
	...
	public static int getPassiveCreature(int type) {
		return DungeonConstants.PASSIVE_CREATURES[type][Utils
				.random(DungeonConstants.PASSIVE_CREATURES[type].length)];
	}

	public static Room getStartRoom(int type, int complexity,
			RoomReference reference, Room[][] map) {
		Room[] possibleRooms = selectPossibleRooms(reference, map,
				DungeonConstants.START_ROOMS[type], true, false, complexity);
		if (possibleRooms.length == 0)
			return null;
		return possibleRooms[Utils.random(possibleRooms.length)];
	}

	public static Room getPuzzleRoom(int type, int complexity,
			RoomReference reference, Room[][] map) {
		Room[] possibleRooms = selectPossibleRooms(reference, map,
				DungeonConstants.PUZZLE_ROOMS[type], true, true, complexity);
		if (possibleRooms.length == 0)
			return null;
		return possibleRooms[Utils.random(possibleRooms.length)];
	}

	public static Room getNormalRoom(int type, int complexity,
			RoomReference reference, Room[][] map) {
		Room[] possibleRooms = selectPossibleRooms(reference, map,
				DungeonConstants.NORMAL_ROOMS[type], true, true, complexity);
		if (possibleRooms.length == 0)
			return null;
		return possibleRooms[Utils.random(possibleRooms.length)];
	}

	public static int getBossRoomInt() {
		return j;
	}

	public static int j = 0;

	public static BossRoom getBossRoom(int type, int complexity,
			int dungeonLevel) {
		BossRoom[] possibleRooms = selectPossibleBossRooms(type, complexity,
				dungeonLevel);
		if (possibleRooms.length == 0)
			return null;
		int l = Utils.random(possibleRooms.length);
		j = l;
		return possibleRooms[l];
	}

	public static BossRoom[] selectPossibleBossRooms(int type, int complexity,
			int dungeonLevel) {
	...
	public static Room[] selectPossibleRooms(RoomReference reference,
			Room[][] map, HandledRoom[] handledRooms, boolean checkBounds,
			boolean checkMap, int complexity) {
		...
		for (HandledRoom handledRoom : handledRooms) {
			...
			for (int rotation = 0; rotation < DungeonConstants.ROTATIONS_COUNT; rotation++) {
				...
				if (checkBounds && !checkDungeonBounds(reference, map, room))
					continue;
				if (checkMap) {
					Room checkRoom = reference.getX() == 0 ? null
							: map[reference.getX() - 1][reference.getY()];
					if (checkRoom != null) {
						if (room.hasWestDoor()) {
					...
					checkRoom = reference.getX() == map.length - 1 ? null
							: map[reference.getX() + 1][reference.getY()];
					if (checkRoom != null) {
						if (room.hasEastDoor()) {
					...
					checkRoom = reference.getY() == 0 ? null : map[reference
							.getX()][reference.getY() - 1];
					if (checkRoom != null) {
