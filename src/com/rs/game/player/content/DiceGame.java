...
public class DiceGame {
	...
	private static void generateRolledNumber(int lowest, int highest, Random r,
			Player player) {
		if (lowest > highest) {
			return;
		}
	...
	private static void sendNumber(Player player, int numberRolled) {
		player.getPackets().sendGameMessage("Rolling...");// can be deleted if
															// you want
		player.setNextForceTalk(new ForceTalk("rolled <col=FF0000>"
				+ numberRolled + "</col> " + "on the percentile dice"));
		player.getPackets().sendGameMessage(
				"rolled <col=FF0000>" + numberRolled + "</col> "
						+ "on the percentile dice");// correct message when
													// number is rolled
	}
}