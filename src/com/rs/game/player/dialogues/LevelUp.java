...
public final class LevelUp extends Dialogue {
	...
	public void start() {
		...
		player.getPackets().sendGameMessage(
				"<col=996600>You've just advanced a" + (name.startsWith("A") ? "n" : "")
						+ " " + name + " level! You have reached level "
						+ level + ".");
		player.getPackets().sendConfigByFile(4757, getIconValue(skill));
		switchFlash(player, skill, false); //change to true if you want it to flash
		...
		if (level == 99 || level == 120)
			for (Player players : World.getPlayers()) {
					players.getPackets().sendGameMessage(
							"<img=8> <col=996600>"
							              + player.getDisplayName() + " has just achieved level 99 in: <col=996600>"
									+ name + "</col>");
		}
		if (level == 99) {
			player.LEVELS_ACHIEVED++;
			
			if (player.LEVELS_ACHIEVED == 100) {
				
					
					for (Player players : World.getPlayers()) {
						players.getPackets().sendGameMessage(
								"<img=8> <col=996600>"
								              + player.getDisplayName() + " has achieved 99 in a skill, over 100 times!");
					}
				}
			}
