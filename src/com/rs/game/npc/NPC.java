...
import com.rs.game.npc.glacor.SecondBar;
...
public class NPC extends Entity implements Serializable {
			...
			if (Misc.random(12) == 1) {
				if (Misc.random(12) == 0)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>Welcome to Dundrew!"));
				
				else if (Misc.random(12) == 1)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>Talk to me if you would like to teleport to some places!"));
				
				else if (Misc.random(12) == 2)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>Remember to type ::commands for the full list"));
				else if (Misc.random(12) == 3)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>PM an available [ADMIN], [MOD], or [SS] for help!"));
				else if (Misc.random(12) == 4)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>Skilling is a great way to make some coin!"));
				else if (Misc.random(12) == 5)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>Voting gives you benefits, and the server."));
				else if (Misc.random(12) == 6)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>A best friend is like a four leaf clover, hard to find, lucky to have."));
				else if (Misc.random(12) == 7)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>I did not trip and fall. I attacked the floor and I believe I am winning."));
				else if (Misc.random(12) == 8)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>It's not important to win, it's important to make the other guy lose."));
				else if (Misc.random(12) == 9)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>Smile today, tomorrow could be worse."));
				else if (Misc.random(12) == 10)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>Never take Dundrew seriously. Nobody gets out alive anyway."));
				else if (Misc.random(12) == 11)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>The road to success is always under construction."));
				else if (Misc.random(12) == 12)
					this.setNextForceTalk(new ForceTalk(
							"<img=4>Sometimes when I close my eyes, I can't see."));
	...
	public void sendDrop(Player player, Drop drop) {
		...
		//all the random and rare drops handled here
		Random charmChance = new Random();
		
		  int roll = charmChance.nextInt(99);
			  if(roll < 18) {
				  World.addGroundItem(new Item(18017), new WorldTile(getCoordFaceX(size),
							getCoordFaceY(size), getPlane()), player, false, 180, true);
			  } else if (roll < 28 && roll > 19) {
				  World.addGroundItem(new Item(12159), new WorldTile(getCoordFaceX(size),
							getCoordFaceY(size), getPlane()), player, false, 180, true);
			  } else if (roll > 94) {
				  World.addGroundItem(new Item(12163), new WorldTile(getCoordFaceX(size),
							getCoordFaceY(size), getPlane()), player, false, 180, true);
			  } else if (roll < 90 && roll > 80) {
				  World.addGroundItem(new Item(12160), new WorldTile(getCoordFaceX(size),
							getCoordFaceY(size), getPlane()), player, false, 180, true);
			  }
			  Random rareChance = new Random();
			  int percent = rareChance.nextInt(99);
				  if(percent == 99) {
					  World.addGroundItem(new Item(405), new WorldTile(getCoordFaceX(size),
								getCoordFaceY(size), getPlane()), player, false, 180, true);
				  } else if (percent < 23 && percent > 19) {
					  World.addGroundItem(new Item(2677), new WorldTile(getCoordFaceX(size),
								getCoordFaceY(size), getPlane()), player, false, 180, true);
				  } else if (percent < 98 && percent > 90 ) {
					  World.addGroundItem(new Item(385), new WorldTile(getCoordFaceX(size),
								getCoordFaceY(size), getPlane()), player, false, 180, true);
				  } else if (percent < 90 && percent > 80) {
					  World.addGroundItem(new Item(3026), new WorldTile(getCoordFaceX(size),
								getCoordFaceY(size), getPlane()), player, false, 180, true);
				  } else if (percent == 1) {
					  World.addGroundItem(new Item(989), new WorldTile(getCoordFaceX(size),
								getCoordFaceY(size), getPlane()), player, false, 180, true);
				  }
				  if (player.NPC_KILLED == 10000) {
						for (Player players : World.getPlayers()) {
							players.getPackets().sendGameMessage(
									"<img=8> <col=996600>"
									              + player.getDisplayName() + " has killed 10,000 Monsters!");
						}
					}
				  int [] charms =
					  {12159, 12160, 12158};
			  ...
			  if ((ringId == 15015) || (ringId == 9104)) {
				  player.getInventory().addItem(charms[li],ratez);
			  }
				  if (getId() == 6215 || getId() == 6211 || getId() == 3406
				|| getId() == 6216 || getId() == 6214 || getId() == 6215
				|| getId() == 6212 || getId() == 6219 || getId() == 6221
				|| getId() == 6218) {
			player.ZamorakKC = +1;
