...
public final class WorldPacketsDecoder extends Decoder {
	...
	// private final static int AFK_PACKET = 85;
		...
		if (packetId == WALKING_PACKET || packetId == MINI_WALKING_PACKET) {
			...
			case 747:
				if (player.getFamiliar() == null)
					return;
				if (npc.getId() == 14301) {
					Glacor glacor = (Glacor) npc;//wtf??? lol
					if (glacor != null) {
						if (glacor.getTarget() == null) {
							if (player.getAttackedByDelay() > Utils
									.currentTimeMillis()) {
								return;
							} else {
	...
	public void processPackets(final int packetId, InputStream stream,
		...
		if (packetId == PING_PACKET) {
				...
				case 59:// High Alch
					...
					if (player.getEquipment().getWeaponId() == 1401
						...
						player.getInventory().addItem(
								995,
								new Item(itemId, 1).getDefinitions().getValue(
										itemId) >> 6);
					} else {
						if (!player.getInventory().containsItem(561, 1)
			 ...
			 * player.getSession().getChannel().close(); } }, 5, 5);
			 */
			/*
			 * } else if (packetId == AFK_PACKET) { if (packetId == 93)//TODO
			 * WorldTasksManager.schedule(new WorldTask() {
			 * 
			 * @Override public void run() {
			 * player.getSession().getChannel().close(); } }, 5, 5);
		...
		} else if (packetId == CLAN_NAME_PACKET) {
			// TODO
			...
			final int id = stream.readUnsignedShort128();
			@SuppressWarnings("unused")
			boolean unknown = stream.readByte() == 1;// Dont delete this.
			int y = stream.readUnsignedShort();
			int x = stream.readUnsignedShortLE();
			final WorldTile tile = new WorldTile(x, y, player.getPlane());
			final int regionId = tile.getRegionId();
			...
			Logger.log(this, "Missing packet " + packetId + ", expected size: "
					+ length + ", actual size: " + PACKET_SIZES[packetId]);
		}
	}
