...
public class InventoryOptionsHandler {
	...
	public static void handleItemOption1(Player player, final int slotId,
		...
		if (itemId == 15090) {
		 ...
		 if (itemId == 405) {
			// player.setNextAnimation(new Animation(11900));
				player.setNextGraphics(new Graphics(1635));
				int[] RandomItems = { 2669,2671,2673,2675,3480,2653,2655,2657,2659,3478,
						2661,2663,2665,2667,3479,3481,3483,3485,3486,3488,19413,19416,19419,
						19422,19425,19428,19431,19434,19437,19440,19398,19401,19404,19407,
						19410,2583,2585,2587,2589,2599,2601,2603,2605,2623,2625,2627,2629,
						7396,7392,10362,10364,10366,10687,2591,2593,2595,2595,2597,2607,2609,
						2611,2613,2615,2617,2619,2621,7362,7366,7386,7390,7394,10316,
						10392,10398,10394,7319,7321,7323,7325,7327,2645,2647,2649,
						2579,2577,10550,15426,13103,2631,3057,6188,7003,13109,13111,13113,13115,
						19275,19278,19281,19284,19287,19290,19293,19296,19299,19302,
						19305,2639,2641,2643,7398,7399,7400,8950,1837,13095,13097,13099,13101,
						3058,3059,3060,3061,7594,7595,7596,7593,7592,5042,5044,5046,6858,6859,6860,6861,
						10836,10837,10838,10839,10840,14595,14602,14603,14605,9921,9922,9923,9924,9925,15352,1037,1419,22321,12634,12645,15673,14076,14077,14081,11789,
						10507,15422,15423,15425,20077,20177,9470,18744,18745,18746,13661,
						10933,10939,10940,10941,6739,15259,20787,20788,20789,20790,10460,10468,10458,10464,10462,10466
				...
				player.getPackets().sendGameMessage("You open the casket, and take whats inside.");
				player.CASKETS_OPENED++;
				
				
				if (player.CASKETS_OPENED == 250) {
					for (Player players : World.getPlayers()) {
						players.getPackets().sendGameMessage(
								"<img=8> <col=996600>"
								              + player.getDisplayName() + " has opened 250 Caskets!");
					}
				}
					return;
		 }
		...
		if (itemId == 3801) { //KEG
			player.getPackets().sendGameMessage("You start to feel dizzy!");
			player.getPackets().sendGameMessage("You have to rest to feel better...");
			player.setNextForceTalk(new ForceTalk("Ima get drunk noww.."));
	...
	public static void handleItemOnItem(final Player player, InputStream stream) {
		...
		if (interfaceId == Inventory.INVENTORY_INTERFACE
			...
			if (itemUsedId == 11286 && itemUsedWithId == 1540) {
				...
				player.sm("You succefully created the Dragonfire Shield.");
				player.SHIELDS_MADE++;
				if (player.SHIELDS_MADE == 50) {
					for (Player players : World.getPlayers()) {
						players.getPackets().sendGameMessage(
								"<img=8> <col=996600>"
								              + player.getDisplayName() + " has created 50 Dragonfire Shields!");
					}
			...
			if (Firemaking.isFiremaking(player, itemUsed, usedWith))
				...
				player.VINE_WHIP_MADE++;
				
				player.sm("Good job, you have succesfully combined a whip and vine into a vine whip.");
				if (player.VINE_WHIP_MADE == 1) {
					
					for (Player players : World.getPlayers()) {
						players.getPackets().sendGameMessage(
								"<img=8> <col=996600>"
								              + player.getDisplayName() + " has created a Vine whip for the first time!"
										);
				}
			} 
