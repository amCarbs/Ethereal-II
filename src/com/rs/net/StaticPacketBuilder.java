...
public class StaticPacketBuilder {
	...
	private void expandCapacity(int minimumCapacity) {
		...
		if (curLength > payload.length)
			curLength = payload.length;
	...
	public StaticPacketBuilder addBytes(Packet data) {
		return addBytes(data.getData(), 0, data.getLength());
	}
	...
	public StaticPacketBuilder addBytesR(byte[] data, int offset, int len) {
	...
	public StaticPacketBuilder addBytesA(byte[] data, int offset, int len) {
	...
	public StaticPacketBuilder writeBytes_reverseA(byte abyte0[], int offset,
			int length) {
	...
	public OutputStream toStream() {
		byte[] data = new byte[curLength];
		System.arraycopy(payload, 0, data, 0, curLength);
		return new OutputStream(id, size, data);
	}
	...
	public StaticPacketBuilder addBytes_Reverse_S(byte[] data, int len,
			int offset) {
		for (int i = len - 1; i >= 0; i--) {
			addByte((byte) (data[i] - 128));
		}
		return this;
	}

	public StaticPacketBuilder addSmart(int i) {
