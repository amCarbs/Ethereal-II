...
public class WorldPacketsEncoder extends Encoder {
	...
	public void resetGe(int slot) {
		player.getSession().write(
				new StaticPacketBuilder().setId(134).addByte((byte) slot)
						.addByte((byte) 0).toStream());
	}
	...
	public void sendJoinClanChat(String username, String clanName) {
		...
		if (!player.inClanChat) {
			...
			player.getPackets().sendGameMessage(
					"You join the " + clanName + " clan chat.");
			player.inClanChat = true;
		} else {
			player.getPackets().sendGameMessage(
					"You leave the " + clanName + " clan chat.");
	...
	public void sendShopLoadMainStock(int shopId) {
		StaticPacketBuilder spb = new StaticPacketBuilder().setId(120)
				.setSize(Size.VariableShort).addString("vg").addInt(shopId)
				.addInt(93).addInt(25);
	...
	public void sendInterface3(int showId, int windowId, int interfaceId,
			int childId) {
		int lastvalue = windowId * 65536 + interfaceId;
		StaticPacketBuilder spb = new StaticPacketBuilder().setId(56)
				.addShort(interfaceId >> 16 | childId).addByteS((byte) showId)
				.addShort(childId).addLEInt(lastvalue);
		player.getSession().write(spb.toStream());
	}
	...
	public void sendAccessMask(int set1, int set2, int interfaceId1,
		...
		spb.addInt(interfaceId2 << 16 | childId2).addLEShort(set2)
				.addLEShort(set1).addLEShortA(0)
	...
	public void sendAccessMask(int set, int inter, int child, int off, int len) {
		StaticPacketBuilder packet = new StaticPacketBuilder().setId(113)
				.addInt(set).addLEShort(len).addLEShort(off).addLEShortA(ID++)
	...
	public void sendItems(int interfaceId, int childId, int type,
		...
		StaticPacketBuilder spb = new StaticPacketBuilder().setId(120).setSize(
				Size.VariableShort);
	...
	public WorldPacketsEncoder setInterfaceConfig(int interfaceId, int childId,
			boolean set) {
		OutputStream stream = new OutputStream();
		stream.setId(3);
		stream.writeShort(0);
	...
	public void sendRunScriptl(int id, Object[] params, String types) {
		if (params.length != types.length())
			return;
		StaticPacketBuilder packet = new StaticPacketBuilder().setId(70)
				.setSize(Size.VariableShort).addShort(ID++).addString(types);
		int idx = 0;
		for (int i = types.length() - 1; i >= 0; i--) {
			if (types.charAt(i) == 's')
	...
	public void sendClanChatChannel() {
		ClanChat manager = player.getClanChat();
		OutputStream stream = new OutputStream(manager == null ? 3
				: manager.getDataBlock().length + 3);
		stream.writePacketVarShort(7);
		if (manager != null)
	...
	/**
	 * Sends the grand exchange progress bar appropriately with the information
	 * provided
	 * 
	 * @param player
	 *            The player sending the paket
	 * @param slot
	 ...
	 * @param amount
	 *            The maximum amount of the item you're buying/selling
	 * @param amountSold
	 *            The progress percentage of the offer being completed
	 */
	public void sendGrandExchangeBar(Player player, int slot, int progress,
			int item, int price, int amountOffered, int amountSold) {
		OutputStream output = new OutputStream();
		output.writePacket(53);
		output.writeByte(slot);
		output.writeByte(progress);
		output.writeShort(item);
		output.writeInt(price);
	...
	public void sendInputIntegerScript(boolean b, String string) {
